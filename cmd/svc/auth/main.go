...
func main() {
	...
	poolConfig, err = svcs.GetServiceConfig("auth")
	if err != nil {
		panic(err)
	}
	...
	if teamAddr == "#" {
		...
		glog.Infof("Starting auth service pool with config: %+v\n", poolConfig)
		...
		hbi.ServeTCP(auth.NewServiceContext, fmt.Sprintf("%s:0", poolConfig.Host), func(listener *net.TCPListener) {
